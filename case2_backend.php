...
set_time_limit(120); // Allow longer execution
...
try {
    ...
    foreach ($isolationLevels as $level) {
        ...
        // Slaves poll (will see old value until commit)
        $slave1Reads = pollSlaveWithTiming($node1['conn'], $userId);
        $slave2Reads = pollSlaveWithTiming($node2['conn'], $userId);

        // Commit master
        $master['conn']->commit();

        // Final reads after commit
        $finalMasterRead = ['time'=>date('H:i:s'),'data'=>readUser($master['conn'],$userId)];
        $finalNode1Read = ['time'=>date('H:i:s'),'data'=>readUser($node1['conn'],$userId)];
        $finalNode2Read = ['time'=>date('H:i:s'),'data'=>readUser($node2['conn'],$userId)];
        ...
        $results[$level] = [
            ...
            'slave1_poll_reads'=>$slave1Reads,
            'slave2_poll_reads'=>$slave2Reads,
            'final_reads'=>[
